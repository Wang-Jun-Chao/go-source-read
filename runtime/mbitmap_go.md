...
// 堆栈，数据和bss位图
//
// data和bss段中的栈帧和全局变量由1位位图描述，其中0表示不关心，1表示在GC期间要访问的实时指针。
// 从低位开始消耗每个字节中的位：1 << 0、1 << 1，依此类推。
//
// 堆位图
//
// 对于堆中每个指针大小的字，堆位图包括2位，存储在支持每个堆竞技场的heapArena元数据中。
// 也就是说，如果ha是开始的竞技场的heapArena，则ha.bitmap[0]保存从start + 3*ptrSize开始的四个pb的2位条目，
// ha.bitmap[1]保存用于以下内容的条目： start + 4*ptrSize到start + 7*ptrSize，依此类推。
//
// 在每个2位条目中，低位持有与1位位图中相同的信息：0表示不关心，1表示在GC期间要访问的实时指针。
// 高位的含义取决于所描述字在其分配对象中的位置。在所有字中，除第二个字外，高位表示该对象仍在描述中。
// 换句话说，如果遇到具有高位0的位对，则低位也可以假定为0，并且对象描述结束。该00称为"死"编码：它表示对象中的其余字对垃圾收集器不感兴趣。
//
// 在第二个字中，高位是GC的"选中标记"位（见下文）。
//
// 2位条目在写入字节时会被拆分，因此字节的上半部分包含4个高位，下半部分包含4个低（指针）位。这种形式允许从1位复制到4位，以保持指针位连续，而不必将它们间隔开。
//
// 代码利用了以下事实：堆位图的零值未设置活动指针位，并且（取决于位置），未使用，未选中标记且为无效编码。修改编码时必须保留这些属性。
//
// 不保留noscan跨度的位图。代码必须在检查其位图之前通过检查范围中的noscan位或查询其类型的信息来确保可扫描对象。
//
// 选中标记
//
// 在并发垃圾收集器中，人们担心由于在没有写障碍或收集器实现中存在错误的情况下发生突变而无法标记活动对象。
// 作为健全性检查，GC具有“检查标记”模式，该模式将在对象全局停机的情况下遍历对象图，以确保应标记的所有内容均已标记。
// 在检查标记模式下，在堆位图中，对象第二个字的2位条目的高位保留检查标记位。不处于检查标记模式时，此位设置为1。
//
// 最小可能的分配是8个字节。在32位计算机上，这意味着每个分配的对象都有两个字，因此复选标记位有空间。
// 但是，在64位计算机上，8字节分配只是一个字，因此第二对位不可用于对复选标记进行编码。
// 但是，由于非指针分配被组合为更大的16字节（maxTinySize）分配，因此普通的8字节分配必须是指针，因此实际上不需要第一个字中的类型位。
// 它通常仍被使用，除了在复选标记中将类型位重新用作复选标记位，然后在完成时将其重新初始化（至1）作为类型位。
//
...
const (
	...
	heapBitsShift      = 1     // shift offset between successive bitPointer or bitScan entries // 在连续的bitPointer或bitScan条目之间移动偏移量
	wordsPerBitmapByte = 8 / 2 // heap words described by one bitmap byte // 堆一个位图字节描述的字

	// all scan/pointer bits in a byte // 一个字节中的所有扫描/指针位
...
func addb(p *byte, n uintptr) *byte {
	...
	// 注意：写出完整表达式而不是调用add（p，n）来减少内联过程中编译器为此琐碎表达式生成的临时信息数量。
...
// heapBits上的方法采用值接收器，以便编译器可以更轻松地内联这些方法的调用并独立注册struct字段。
type heapBits struct {
	bitp  *uint8
	shift uint32
	arena uint32 // Index of heap arena containing bitp // 包含bitp的堆竞技场的索引
	last  *uint8 // Last byte arena's bitmap // 竞技场的位图的最后一个字节
}

// Make the compiler check that heapBits.arena is large enough to hold
// the maximum arena frame number.
// 使编译器检查heapBits.arena是否足够大以容纳最大的竞技场帧号。
...
// marking purposes.
//
// markBits提供对堆中对象的标记位的访问。 bytep指向保存标记位的字节。
// mask是一个设置了单个位的字节，可以与* bytep进行＆ed查看该位是否已设置。
// *m.byte＆m.mask！= 0表示标记位已设置。 index可以与span信息一起使用，以生成堆中对象的地址。
// 我们维护一组标记位以进行分配，并保留一组标记位。
...
// s.allocCache.
// refillAllocCache从whereByte开始占用8个字节的s.allocBits并对它们求反，
// 以便可以使用ctz（计数尾随零）指令。然后，将这8个字节放入缓存的64位s.allocCache中。
